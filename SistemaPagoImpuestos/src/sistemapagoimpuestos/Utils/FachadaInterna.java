...
//    }
    ...
    //USAR ESTE ESTE BUSCAR
    public List<Object> buscar (Class claseABuscar, List<DTOCriterio> criterioList)
    {
        Criteria cr = null;
        try
        {
            cr = HibernateUtil.getSession().createCriteria(Class.forName(claseABuscar.getName()), claseABuscar.getName().toLowerCase());
        }
        catch (ClassNotFoundException e)
        {
            System.out.println("Error creating criteria. " + e);
                ...
                switch (operacion)
                {
                    ...
                    case "contains":
                        ...
                        String lista = String.valueOf(atributo.charAt(0))
                                .toLowerCase() + atributo.substring(1) + "List"; //materiaList, detalleReposicionList
                        cr.createAlias(lista, "lista")
                                .add(Restrictions.conjunction(Restrictions.eq("lista.OID" + claseABuscar.getName(), ((Entity) valor).getOID())));
                        break;
                }
